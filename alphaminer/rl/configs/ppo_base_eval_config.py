...
trading_ppo_config = dict(
    ...
    env=dict(
        ...
        stop_value=1000.,  # stop when the policy doesn't lose money
        manager=dict(shared_memory=False, ),  # True doesn't work
        max_episode_steps=None,  # None means backtest on full data
        cash=1000000,
    ...
    policy=dict(
        cuda=True,
        recompute_adv=True,
        load_path="../configs/trading_ppo_seed0/ckpt/iteration_0.pth.tar",
        model=dict(
